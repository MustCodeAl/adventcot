macro_rules! match_float_ranges {
    ($input: expr; $($range: expr => $arm: expr,)* _ => $tail: expr,) => {
        match $input {
            $(x if $range.contains(&x) => $arm,)*
            _ => $tail,
        }
    }
}
...
fn main() {
    // let a = 24.0;
    // match_float_ranges!{ 11.0f32;
    //     (0.0..=5.0) => println!("five or less"),
    //     (5.0..a) => {println!("less than a")},
    //     _ => println!("moar!!"),
    // }

    let output= advent_func(message.to_string());
let mut number = 0;


let mut number_output= Vec::new();
for nuumus in output  {
    let mut bumber = String::new();
    
    let numstr =  nuumus.to_string();

//   if numstr.len() == 2 {

//             let mystr = numstr.chars().rev().collect::<String>();
//             bumber.push_str(mystr.as_str());
//         } else {

            bumber.push(numstr.chars().last().unwrap());
            bumber.push(numstr.chars().nth(0).unwrap());

        // }

    number_output.push(bumber);
    
}
for num in number_output.clone() {
    number+=num.trim().parse::<u32>().expect("failed to get integer");
}


    println!("test for aoc {:?}, {}", number_output, number);
...
fn advent_func(advent_msg: String) -> Vec<u32> {
    ...
    for line in advent_msg.lines() {
        let mut msg = String::new();


        for anum in alphanums {
            if line.contains(anum) {
                if line.find(anum).unwrap() != 0 {
                    match anum {
                        "one" => msg.push('1'),
                        "two" => msg.push('2'),
                        "three" => msg.push('3'),
                        ...
                        "eight" => msg.push('8'),
                        "nine" => msg.push('9'),
                        _ => println!("not avaliable number"),
                    }
        ...
        let num = msg.trim().parse::<u32>().expect("failed to get number");
